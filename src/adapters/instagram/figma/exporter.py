...
class CarouselExporter:
    """Exports Figma frames as images and assembles a ``Carousel`` content model."""
    ...
    async def export(
        ...
        """Export frames from a Figma file and return a ``Carousel``.
        ...
        for idx, export in enumerate(exports):
            ...
            slide = CarouselSlide(
                media=MediaAsset(uri=str(dest.resolve()), mime_type=mime),
            )
            slides.append(slide)

        if len(slides) < 2:
            raise ValueError(
                f"Only {len(slides)} slides exported successfully; need at least 2"
            )
