...
async def _run_single_command(entry: _BatchEntry) -> CliResult:
    ...
    if result.exit_code == 0:
        ...
        hub.publish(
            entry.item_id,
            ProgressEvent(event_type="error", message=f"Failed: {error[:200]}"),
        )
...
async def _run_reddit_two_step(entry: _BatchEntry) -> None:
    ...
    hub.publish(
        ...
        ProgressEvent(
            event_type="phase",
            message="Discovering and generating comments...",
            phase="discovery",
        ),
    )
    ...
    if gen_result.exit_code != 0:
        ...
        await db.update_content_status(
            ...
            error_message=error,
            preview_data={
                "stdout": gen_result.stdout[:3000],
                "stderr": gen_result.stderr[:3000],
            },
        )
        hub.publish(
            entry.item_id,
    ...
    if pub_result.exit_code == 0:
        ...
        await db.update_content_status(
            ...
            error_message=error,
            preview_data={
                "stdout": pub_result.stdout[:3000],
                "stderr": pub_result.stderr[:3000],
            },
        )
        hub.publish(
            entry.item_id,
